...
% in this first agent follows virtual agent 2' and rest all i agents follow their i+1 agent
...
k1=.05;               % {k1 is control gain .For a particular initial condition ,for some range of values of k a circle will be formed around the target,but for other values of k a circle will not be formed}%
...
for i=2:100000        %{iterating 100000 times }%
        for j=1:n
               ...
               else       % {as only 1 follows 2' and rest all other agent follows their i+1 agent}%                             xvir=x(mod(j,n)+1);
                    yvir=y(mod(j,n)+1);
               end
